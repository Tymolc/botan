   ...
   public:
      explicit Bzip2_Compression_Stream(size_t block_size)
         ...
         */
         ...
         if(rc != BZ_OK)
            throw Compression_Error("BZ2_bzCompressInit", rc);
         }
      ...
      bool run(uint32_t flags) override
         ...
         if(rc < 0)
            throw Compression_Error("BZ2_bzCompress", rc);
...
class Bzip2_Decompression_Stream final : public Bzip2_Stream
   ...
   public:
      Bzip2_Decompression_Stream()
         ...
         if(rc != BZ_OK)
            throw Compression_Error("BZ2_bzDecompressInit", rc);
         }
      ...
      bool run(uint32_t) override
         ...
         if(rc != BZ_OK && rc != BZ_STREAM_END)
            throw Compression_Error("BZ2_bzDecompress", rc);
