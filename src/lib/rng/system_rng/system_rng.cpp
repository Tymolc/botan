...
class System_RNG_Impl final : public RandomNumberGenerator
   ...
   public:
      ...
      void randomize(uint8_t buf[], size_t len) override
         ...
         if(!success)
            throw Runtime_Error("RtlGenRandom failed");
         }
   ...
   public:
      System_RNG_Impl()
         ...
         if(ret != STATUS_SUCCESS)
            throw Runtime_Error("System_RNG failed to acquire crypto provider", ret);
         }
      ...
      void randomize(uint8_t buf[], size_t len) override
         ...
         if(ret != STATUS_SUCCESS)
            throw Runtime_Error("System_RNG call to BCryptGenRandom failed", ret);
         }
      ...
      void randomize(uint8_t buf[], size_t len) override
         ...
         while(len > 0)
            ...
            if(got < 0)
               ...
               if(errno == EINTR)
                  continue;
               throw Runtime_Error("System_RNG getrandom failed", errno);
               }
...
class System_RNG_Impl final : public RandomNumberGenerator
   ...
   public:
      System_RNG_Impl()
         ...
         if(m_fd < 0)
            throw Runtime_Error("System_RNG failed to open RNG device", errno);
         }
...
void System_RNG_Impl::randomize(uint8_t buf[], size_t len)
   ...
   while(len)
      ...
      if(got < 0)
         ...
         if(errno == EINTR)
            continue;
         throw Runtime_Error("System_RNG read failed", errno);
         }
      if(got == 0)
         throw Runtime_Error("System_RNG EOF on device"); // ?!?
   ...
   while(len)
	 ...
	 * writing.
         */
         ...
         throw Runtime_Error("System_RNG write failed", errno);
         }
