   ...
   public:
      Asio_Socket(const std::string& hostname,
         ...
         if(ec || m_tcp.is_open() == false)
            throw Runtime_Error("Connection to host " + hostname + " failed");
         }
...
class BSD_Socket final : public OS::Socket
   ...
   private:
      ...
      static void socket_init()
         ...
         if (::WSAStartup(wsa_version, &wsa_data) != 0)
            {
            throw Runtime_Error("WSAStartup() failed", WSAGetLastError());
            }
      ...
      static void set_nonblocking(socket_type s)
         ...
         if(::fcntl(s, F_SETFL, O_NONBLOCK) < 0)
            throw Runtime_Error("Setting socket to non-blocking state failed", errno);
         }
   ...
   public:
      BSD_Socket(const std::string& hostname,
         ...
         if(rc != 0)
            {
            throw Runtime_Error("Name resolution failed for " + hostname, rc);
            }
               ...
               if(nonblocking_connect_in_progress())
                  ...
                  if(active)
                     ...
                     if(::getsockopt(m_socket, SOL_SOCKET, SO_ERROR, reinterpret_cast<char*>(&socket_error), &len) < 0)
                        throw Runtime_Error("Error calling getsockopt", errno);
         ...
         if(m_socket == invalid_socket())
            {
            throw Runtime_Error("Connecting to " + hostname +
      ...
      void write(const uint8_t buf[], size_t len) override
         ...
         while(sent_so_far != len)
            ...
            if(active == 0)
               throw Runtime_Error("Timeout during socket write");

            const size_t left = len - sent_so_far;
            socket_op_ret_type sent = ::send(m_socket, cast_uint8_ptr_to_char(&buf[sent_so_far]), left, 0);
            if(sent < 0)
               throw Runtime_Error("Socket write failed", errno);
            else
      ...
      size_t read(uint8_t buf[], size_t len) override
         ...
         if(active == 0)
            throw Runtime_Error("Timeout during socket read");

         socket_op_ret_type got = ::recv(m_socket, cast_uint8_ptr_to_char(buf), len, 0);

         if(got < 0)
            throw Runtime_Error("Socket read failed", errno);
